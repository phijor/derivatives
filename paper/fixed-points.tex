...
Traditionally, the (smallest) fixed point associated to a signature container is constructed as follows:
...
\begin{problem}[note={\( \Subst{F}{\Blank} \)-algebras}]
  ...
  \begin{itemize}
    ...
    \item morphisms \( \Alg{\Subst{F}{\Blank}}((G, g), (H, h)) \) are container morphisms
      \( f : \Cart{G}{H} \) together with a path \( f_\sharp : f \circ g = h \circ \Subst{F}{f} \) ensuring that the square
      \[
        \begin{tikzcd}
          \Subst{F}{G} \ar[r, -multimap, "\Subst{F}{f}", ""{swap,name=Ff}] & \Subst{F}{H} \\
          G \ar[r, -multimap, "f"{swap}, ""{name=f}] & H
          \ar[from=1-1, to=2-1, -multimap, "g"{swap}]
          \ar[from=1-2, to=2-2, -multimap, "h"]
