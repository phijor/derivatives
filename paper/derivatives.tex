...
Morphisms of containers compose as expected, and together with an identity morphism \( \id_F : \Cart{F}{F} \)
...
The canonical map taking paths of containers \( F = G \) to categorical isomorphisms,\footnote{%
  that is, pairs \( f : \Cart{F}{G}, g : \Cart{G}{F} \) with chosen paths \( fg = \id_F \) and \( gf = \id_G \)
}
...
In particular, there is a position \( u_s^{\Inv}(\Inr(\bullet)) : G_{\Ps}(fs) \) in the output
...
Denote by \( \CYo(A) \DefEq (\MkCont{1}{A}) \) the container of \enquote{\( A \)-tuples}.
We see that \( \Der^n(G) \) encodes a type of \( G \)-terms with \( n \) holes:
\begin{corollary}
  For all \( n : \mathbb{N} \), there is an adjunction \( \Blank \CTimes \CYo{[n]} \dashv \Der^n \)
