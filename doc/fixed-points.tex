...
Traditionally, the (smallest) fixed point associated to an signature container is constructed as follow:
...
which should make it amenable to generalization.

In light of this, we are going to define a notion fixed point entirely internal to the wild category of containers,
which should nonetheless correspond to the more traditional definition, barring the above-mentioned issues.
...
\begin{definition}
    ...
    \end{align*}
\end{definition}
Substitution into a fixed \( F : \Cont_{I+1} \), \( \Subst{F}{\Blank} \) behaves functorially:
\begin{problem}
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    It remains to show that for any other algebra morphism \( \alpha^* : \Cart{\mu F}{G} \), \( \Op{rec}_F(\alpha) = \alpha^* \).
    \todo[inline]{Finish this by sketching the proof}
  \end{proof}
\end{theorem}
...
Having convinced ourselves that fixed points do exist, we can now characterize their derivatives.
Let \( F : \Cont_2 \).
