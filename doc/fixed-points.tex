...
\begin{example}
  ...
  It has two shapes, \( \Op{nil} \) and \( \Op{cons} \),
  over which there are positions
  \begin{align*}
    L_{\Ps}(\Op{nil}, \Op{x}) &\DefEq 0 & L_{\Ps}(\Op{cons}, \Op{x}) &\DefEq 1 \\
    L_{\Ps}(\Op{nil}, \Op{y}) &\DefEq 0 & L_{\Ps}(\Op{cons}, \Op{y}) &\DefEq 1 \qedhere
  \end{align*}
\end{example}
  ...
  \[
    \begin{tikzcd}
      \Subst{F}{G} \ar[r, -multimap, "\Subst{F}{f}", ""{swap,name=Ff}]
        & \Subst{F}{H} \ar[r, -multimap, "\Subst{F}{\Op{id}}", ""{swap,name=Fid}]
        & \Subst{F}{H}  \\
      G \ar[r, -multimap, "f"{swap}, ""{name=f}]
        & H \ar[r, -multimap, "\Op{id}"{swap}, ""{name=id}]
        & H
      \ar[from=1-1, to=2-1, -multimap, "h"{swap}]
      \ar[from=1-2, to=2-2, -multimap, "g"]
      \ar[from=1-3, to=2-3, -multimap, "g"]
      \ar[from=Ff, to=f, phantom, "{\scriptstyle f_\sharp}"]
      \ar[from=Fid, to=id, phantom, "{\scriptstyle \Op{id}_\sharp}"]
    \end{tikzcd}
    =
    \begin{tikzcd}
      \Subst{F}{G} \ar[r, -multimap, "\Subst{F}{f}", ""{swap,name=Ff}] & \Subst{F}{H} \\
...
\begin{align*}
  \operatorname{\Op{W-in}} &: \sum_{a : A} (B(a) \to \W(A, B)) \simeq \W(A, B)
    \\
  \operatorname{\Op{\bar{W}-in}}_{a, f} &: C(a) + \smashoperator{\sum_{b : B(a)}} \bar{\W}_{A,B,C}(f(b)) \simeq \bar{\W}_{A,B,C}(\Sup(a, f))
\end{align*}
for all \( a : A \) and \( f : B(a) \to \W(A, B)\).
...
\begin{lemma}\label{is-embedding-W-rec}
  ...
  \begin{proof}
    ...
    \begin{align*}
      ...
      \intertext{
        Unfolding \( w : \W(S, P) \) via \( \operatorname{\Op{W-in}} \) and expanding \( x \),
        we obtain
      }
        &\simeq
...
\begin{lemma}
  ...
  \begin{proof}
    By induction on \( \W(A, B) \) and unfolding via \( \operatorname{\Op{\bar{W}-in}} \).
  \end{proof}
\end{lemma}
