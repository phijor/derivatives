...
\emph{Post hoc} this path has to be unique, since it originates from an isolated point.
...
We would like to characterize the isolated points of \( \Sigma \)-types.
\Autoref{is-isolated-pair} tells us that a pair is isolated if its components are,
and if we had an inverse to \( \SigmaIsolate \), then the converse would hold as well.
...
\begin{problem}
  Define a wild adjunction \( (\eta, \varepsilon) : (\Maybe{-}) \vdash \Der{(-)} \),
  that is:
  \begin{enumerate}
    \item
...
\begin{theorem}
  In the 1-category of set-truncated containers, \( \Der \) is right-adjoint to tensoring with \( \Id \).
\end{theorem}
