...
describe Hedberg's theorem as a global version of \autoref{is-prop-isolated-path}\todo{\cite{Kraus2015TruncationLevelsHomotopy} has a nice discussion in \S 3.2.}:
...
\begin{proposition}[note={Isolated points form a set}]\label{is-set-isolated}
  ...
  \begin{proof}
    ...
    --- after all, its points come with a way to decide equality ---
    hence its path types must be propositions.
  \end{proof}
\end{proposition}

% Alternatively, we can use the forgetful map \( \Isolated{A} \to A \) to classify discrete types:
% \begin{lemma}\label{is-equiv-forget-isolated-iff-discrete}
%   A type \( A \) is discrete if and only if \( \Op{fst} : \Isolated{A} \to A \) is an equivalence.
% \end{lemma}
...
and we believe that it should be straightforward (albeit tedious) to show that \( \Der \) restricts to a pseudo\-functor on this bicategory.
