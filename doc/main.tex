...
\begin{proposition}\label{discrete-is-equiv-sigma-isolated}
  ...
  \begin{proof}
    ...
    \[
      ...
      \end{tikzcd}
    \]
    hence \( \SigmaIsolate \) is an equivalence by the \emph{3-for-2}-property.
  \end{proof}
\end{proposition}

\Autoref{is-isolated-pair} tells us that a pair is isolated if its components are.
It turns out that the converse is true exactly when \( \SigmaIsolate \) is an equivalence:

\begin{lemma}\label{is-equiv-sigma-isolate-iff-isolated-pair}
  Let \( A : \Type \) and \( B : A \to \Type \).
  The following are equivalent:
  \begin{enumerate}
    \item \( \SigmaIsolate_{A,B} \) is an equivalence.
    \item For all \( a_0 : A \) and \( b_0 : B(a_0) \),
...
% However, globally assuming that isolated points of dependent pairs can be characterized this
% way is a very classical property.
% Indeed, we can use the following lemma to show that any type \( A : \Type \) for which this holds,
...
%     Let \( B : A \to \Type \), \( a_0 : A \), \( b_0 : B(a_0) \).
%     It remains to show that, if \( (a_0, b_0) \) is isolated, so is \( a_0 \).
%     But this we can conclude from \autoref{is-equiv-sigma-isolate-pair}.
%   \end{proof}
% \end{proposition}

Taking these results together,
we obtain a way to characterize when a type is discrete in terms of \( \SigmaIsolate \) being an equivalence.
This will be useful in situations in which we have control over the types at which index \( \SigmaIsolate \).
...
Whenever \( a_0 \) is isolated in \( A \), a number of useful properties hold.
There is an obvious map \( \Replace_{a_0} : (A \setminus a_0) + 1 \to A \), defined by cases
as
\(
...
the inverse simply maps \( a_0 \) to some fresh element.
...
We observe that we can replicate the classical behavior only when \( a_0 \) is an isolated point:
...
\begin{proposition}\label{isolated-minus-plus-equiv}
  ...
  \begin{proof}
    ...
    We construct an two-sided inverse \( g : A \to (A \setminus a_0) + 1 \) as follows.
...
\begin{problem}\label{sum-remove-equiv}
  ...
  \begin{align*}
    (A \setminus a_0) + B &\simeq (A + B) \setminus \Inl(a_0) \\
    A + (B \setminus b_0) &\simeq (A + B) \setminus \Inr(b_0)
  \end{align*}
  for isolated points \( a_0 : \Isolated{A} \) and \( b_0 : \Isolated{B} \), respectively.
\end{problem}
...
\begin{problem}
  ...
  \[
    \Graft : \prod_{\Isolated{a} : \Isolated{A}}
  ...
  \begin{construction}
    ...
    Decide equality with \( \Isolated{a} \) to define \( \Graft_{\Isolated{a}}(f, b_0) : A \to B \) as follows:
    \[
      \Graft_{\Isolated{a}}(f, b_0) \DefEq
      \lambda a.\,
      \begin{cases}
...
If \( \Isolated{a} : \Isolated{A} \) is understood from context, write \( \GraftSyntax{f}{b} \DefEq \Graft_{\Isolated{a}}(f, b) \).
...
from the act of defining an interesting map \( f' : F' \to G' \).
...
\begin{proposition}\label{sum-product-rule}\label{sum-rule}\label{leibniz-rule}
  ...
  \begin{enumerate}
    ...
    \item Leibniz rule: \( {\Der{(F \otimes G)}} \CartEquiv {(\Der{F} \otimes G) \oplus (F \otimes \Der{G})} \)
  \end{enumerate}
  \begin{proof}
    Let \( F \JudgeEq (\MkCont{S}{P}) \) and \( G \JudgeEq (\MkCont{T}{Q}) \).
...
\begin{proposition}\label{strong-chain-rule-iff-is-equiv-sigma-isolate}
  ...
  \begin{enumerate}
    \item \( \Op{chain}_{F,G} \) is an equivalence of containers
    \item \( \SigmaIsolate_{P_s,Q_{f(\shortminus)}} \) is an equivalence for all \( s : S \) and \( f : P_s \to T \)
  \end{enumerate}
\end{proposition}
...
\begin{proposition}[note={Lax chain rule for binary containers}]\label{binary-chain-rule}
  ...
  \begin{construction}
    ...
    \]
    Since \( p_0 \) is isolated, this is an instance of \autoref{sum-remove-equiv}.
    ...
    \begin{align}
      R_2(f_2(s , f , \Inr(p_1, q)))
        ...
        &\mathrel{\JudgeEq}
          ...
          \notag
    \end{align}
    This again is justified since both \( p_1 \) and \( q \) are isolated points:
    together, the pair \( (p_1, q) \) is isolated in \( \sum_{p : P_1(s)} Q(f p) \) (cf. \autoref{is-isolated-pair}),
    thus in \eqref{binary-chain-rule-pos-equiv-sum-minus}, we move it to the right of the sum (\autoref{sum-remove-equiv}),
    and in \eqref{binary-chain-rule-pos-equiv-sigma-minus} we split the \( \Sigma \)-type (\autoref{is-equiv-sigma-remove}).
  \end{construction}
\end{proposition}
...
\end{problem}
  \begin{construction}
    ...
    First note that substitution into \( G \) replaces \( \Proj{1} \), i.e.\@ there is an equivalence
    \[
      \gamma_{\textcolor{cyan}{Y}} :
      G[\textcolor{cyan}{Y}]
        \mathrel{\CartEquiv}
      { \Der_0{F}[ {\mu F} ] } \CPlus ( {\Der_1{F}[ {\mu F} ]} \CTimes \textcolor{cyan}{Y} )
    \]
    Equipped with this knowledge, we use the chain rule to define \( \alpha \) as the following composite:
    \begin{equation*}
      \begin{tikzcd}[column sep=large]
        {G[\textcolor{cyan}{\Der(\mu F)}]}
          &
        {\Der(\mu F)}
          \\
        {
          ...
          (
            {\Der_1{F}[ {\mu F} ]}
              \CTimes
            \textcolor{cyan}{\Der(\mu F)}
          )
        }
          &
        \Der(F[\mu F])
        \ar[from=1-1, to=2-1, -multimap, "\gamma_{\textcolor{cyan}{\Der(\mu F)}}"{swap}, "\sim"]
...
\begin{conjecture}
  The \( \mu \)-rule is an equivalence for \emph{any} container.
  \begin{proof}[Idea]
    Let \( F \JudgeEq (\MkCont{S}{P}) \) and \( G \JudgeEq (\MkCont{T}{Q}) \).
    The morphism \( \Op{chain}_{F,G} \) is an equivalence if and only if \( \Op{\SigmaIsolate}_{P_1(s),Q(f \shortminus)} \) is an equivalence for all \( s : S \), \( f : P_1(s) \to T \).

    In the \( \mu \)-rule we instantiate the chain rule for \( F \) and \( \mu F \),
    so the resulting morphism \( \alpha \) is an equivalence of containers iff \( \Op{chain}_{F, {\mu F}} \) is,
    which is equivalent to
    \[
      \prod_{s : S} \prod_{f : P_1(s) \to \W(S, P_1)} \Op{isEquiv}(\SigmaIsolate_{P_1(s), \bar{\W}(S, P_1, P_0) \circ f})
    \]
    Curry this\ldots
    \[
      \prod\nolimits_{(s , f) : \sum_{s : S} P_1(s) \to \W(S, P_1)} \Op{isEquiv}(\SigmaIsolate_{P_1(s), \bar{\W}(S, P_1, P_0) \circ f})
    \]
    Apply \( \Op{in}_{\W} \):
    \[
      \prod_{w : \W(S, P_1)} \Op{isEquiv}(\SigmaIsolate_{P_1(s), \bar{\W}(S, P_1, P_0) \circ \Op{sup}^{\Inv}(w)})
    \]
    Can we prove (by recursion on \( \W \)) that this is always the case?
    After all, \( \W \) and \( \bar{\W} \) are just big iterated \( \Sigma \)-types,
    perhaps we can bootstrap this by characterizing their isolated points.

    If that's not the case, maybe that works exactly for discrete containers?

    Even still, this characterizes when \( \alpha \) is an equivalence; is that preserved by \( \Op{rec} \)?
  \end{proof}
\end{conjecture}
