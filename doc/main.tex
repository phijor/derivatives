...
For sums of discrete types over a discrete base, we can always trivially separate isolated pairs into isolated components:
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    \( \lambda b.\, (a, b) \) is an embedding, which necessarily reflects isolated points.
  \end{proof}
\end{proposition}
...
hence they are closed under products \( \times \), sums \( + \) and substitution \( \Blank[\Blank] \),
see \autoref{container-operations}.
The constant container at a type \( S \) is \( \CConst{S} \DefEq (\MkCont{S}{0}) \);
products and sums are monoidal with units \( \CConst{1} \) and \( \CConst{0} \).
...
The derivative of a containers \( G \) represents a type of
...
\( \Der \) extends to an endofunctor on containers,
and one is interested in finding --- perhaps on a suitable subcategory --- an adjunction \( \Maybe{(-)} \dashv \Der \)
...
\begin{itemize}
  \item Unlike \cite{AbbottEtAl2005DataDifferentiating}, we define the adjunction
    via unit and counit, not as a natural hom-set isomorphism.
  \item
    Mostly do this because it is easier,
    but also because it yields an equivalence of hom-types more directly.
\end{itemize}
...
Presenting the above wild adjunction in terms of unit and counit natural transformations
has two advantages:
Firstly, this data yields the desired equivalence of hom-types
\begin{align*}
  {\Blank}^\sharp &: (\Cart{F}{\Der{G}}) \simeq (\Cart{\Maybe{F}}{G}) \\
  f^{\sharp} &\DefEq \varepsilon_G \circ (\Maybe{f})
\end{align*}
as it would for an 1-categorical adjunction,
which otherwise would be somewhat tedious to establish.
Secondly, \( {\Blank}^{\sharp} \) being an equivalence
...
\begin{itemize}
  \item
    derivative of a container enjoys a number of desirable properties
  \item
    ensure that our generalization still respects those
\end{itemize}

Recall that the \emph{constant container} at a type \( A \) is \( \CConst(A) \DefEq (\MkCont{A}{0}) \).
The derivative of a constant is always the zero-container \( \CConst(0) \),
and that of the identity \( \Id \) is the constant \( \CConst(1) \).
Both facts factor through the following observation:
\begin{proposition}\label{derivative-prop-trunc}
  ...
  and \( \Der(\CConst(A)) \CartEquiv \CConst(0) \).
  \begin{proof}
...
and that for products it obeys a Leibniz rule:
