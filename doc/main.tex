...
% A custom tikzcd arrowtip, a lollipop (⊸).
...
\NewDocumentCommand{\CartEquiv}{}{\SimMultiMap}
...
the inverse simply replaces \( a_0 \) with a fresh element.
In a univalent setting however, \( a_0 \) might have interesting (higher) loop spaces \( \Omega^n(a_0) \),
...
We observe that we can replicate the classical behaviour only when \( a_0 \) is an isolated point:
...
Let \( \Op{FinSet} \DefEq \sum_{X : \Type} \exists n : \mathbb{N}.\, X \simeq \Op{Fin}(n) \) be the universe of Bishop-finite sets,
together with its forgetful map \( \Op{El} \DefEq \Op{fst} : \Op{FinSet} \to \Type \).
\begin{definition}
  The (large) container of \emph{bags} is
  \( \Op{Bag} \DefEq (\MkCont{\Op{FinSet}}{\Op{El}}) \).
\end{definition}
...
\begin{theorem}
  ...
  \[
    \big( \smashoperator{\prod_{F, G : \Cont}} \Op{isEquiv}(\Op{chain}_{F,G}) \big)
      \to
    \smashoperator{\prod_{A : \Type}} \Op{Discrete}(A)
  \]
  \begin{proof}
...
\[
  \Sup : \prod_{a : A} (B(a) \to W(A, B)) \to \W(A, B)
\]
A cursor to a position into a \( \W(A, B) \)-tree, labelled by some \( C : A \to \Type \) can be defined as an inductive family
\( \bar{\W}_{A,B,C} : \W(A,B) \to \Type \) with constructors
\begin{align*}
...
\begin{proposition}[note={Lax chain rule for binary containers}]\label{binary-chain-rule}
  ...
  \begin{construction}
    ...
    As in \autoref{lax-chain-rule}, construct \( \eta_{\Sh} : U_1 \to U_2 \) from \( \SigmaIsolate_{P_1(s),Q(f\shortminus)} \).
    On positions,
    \(
...
\end{problem}
  ...
  \end{construction}
\begin{remark}
  Written in variable style:
  \[
    \Cart%
      {\mu Y.\, \Der_0{F(\mu F(X))} \CPlus (\Der_1{F(\mu F(X))} \CTimes Y)}%
      {\Der(\mu Y. F(X, Y))}%
  \]
\end{remark}
