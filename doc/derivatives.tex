...
ideally by deriving it from the chain-rule.
...
To do define and reason about an operation \( \mu \) taking a container to a fixed-point,
...
Cartesian morphisms between containers \(F , G : \Cont_{I} \) are defined similarly by ranging over all indices \( I \):
\[
  \Cart{F}{G} \DefEq \smashoperator[l]{\sum_{f : F_{\Sh} \to G_{\Sh}}} \prod_{i : I, s : F_{\Sh}} G_{\Ps}(i, f(s)) \simeq F_{\Ps}(i, s)
\]
Together, \( I \)-indexed containers assemble again assemble into a wild category, \( \ContCart_{I} \).
As expected, indexed containers are closed under constants \( \CConst(A) \), sums \( (\CPlus) \) and products \( (\CTimes) \);
Substitution generalizes to an operation that places a container inside the positions at index \( \Nothing : I + 1 \) of a \( I + 1 \)-ary container:
\begin{definition}
    Substitution is an operation \( \Subst{\Blank}{\Blank} : \Cont_{I+1} \to \Cont_{I} \to \Cont_{I} \),
    defined as follows:
    \begin{align*}
      \Subst{(\MkCont{S}{P})}{(\MkCont{T}{Q})}_{\Sh} &\DefEq \sum_{s : S} P(i, s) \to T
        \\
      \Subst{(\MkCont{S}{P})}{(\MkCont{T}{Q})}_{\Ps} &\DefEq \lambda (s, f).\, P(\Just(i), s) + \smashoperator{\sum_{p : P(\Nothing, s)}} Q(i, fp)
        \qedhere
    \end{align*}
\end{definition}
For any fixed \( F : \Cont_{I+1} \), \( F[-] \) is a wild endofunctor of \( I \)-ary containers.
Assuming we have access to \( \W \)-types, we encode inductive types as smallest fixed-points of this functor:
For all \( F : \Cont_{I+1} \), there is a container \( {\mu F} : \Cont_I \)
such that \( \Subst{F}{\mu F} \CartEquiv {\mu F} \).

Recall that for \( A : \Type \) and \( B : A \to \Type \), the type \( \W(A, B) \) has the single constructor
\begin{center}
  \hspace*{\fill}
  {
    \AxiomC{\( a : A \)}
  ...
  \hspace*{\fill}
\end{center}
which defines an equivalence \( \operatorname{\Op{W-in}} : \sum_{a : A} (B(a) \to \W(A, B)) \simeq \W(A, B) \).
A path to a branch inside a \( \W(A, B) \)-tree,
...
We can now define derivatives of indexed containers, and investigate how they behave with respect to fixed-points.
...
\]
For \( i : I \), the \( i \)th projection container is \( \pi_i \DefEq (\MkCont{1}{\lambda j.\,{i = j}}) : \Cont_I \).
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    Using this map, we then prove that all fibers of the shape map are inhabited.
    It remains to show that these fibers are propositions:
    By \( \W \)-induction, we see that they are equivalent to (disjoint) sums of fibers over the constructors \( \Op{top} \)
    and \( \Op{below} \), which are embeddings, hence have propositional fibers.
  \end{proof}
\end{proposition}
