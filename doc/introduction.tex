...
\begin{todo-block}
  Include a link to the formalization and discuss.
\end{todo-block}
...
This is different from the usual approach in which \( \mu F \) is defined in terms of an associated container functor \( \Ext{F} \) of sets or types \cite{AbbottEtAl2005ContainersConstructingstrictly,DamatoEtAl2025FormalisingInductiveCoinductive}.
We do so to avoid coherence problems in the translation from containers to container functors.
We then construct a lax \( \mu \)-rule as an embedding of a fixed point \( \mu F' \) into \( \Der(\mu F) \), in which \( F' \) is derived naturally from \( F \).
