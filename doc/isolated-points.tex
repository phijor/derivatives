\section{Removing Points in Univalent Foundations}
...
\begin{definition}
  ...
  That is, isolated points satisfy the predicate \( \Op{isIsolated} : A \to \Type \),
  \[
    \Op{isIsolated}(a) \DefEq \prod_{b : A} \Op{Dec}(a = b)
  \]
  We denote by
  \(
    \Isolated{A} \DefEq \sum_{a : A} \Op{isIsolated}(a)
  \)
...
\begin{corollary}\label{is-prop-isolated-dec-path}\label{is-prop-is-isolated}
  ...
  In particular, \( \Op{isIsolated}(a) \) is a proposition for all \( a : A \).
\end{corollary}
...
\begin{proposition}\label{is-isolated-pair}\label{sigma-isolate}
  ...
  \begin{proof}
    ...
    it suffices to decide the equivalent type \( \sum_{p : a_0 = a} b_0 = p^{\Inv}_*(b_0) \).
    If \( a_0 \neq a \), then this type is empty.
    Otherwise, we have some \( p : a_0 = a \), and the type is inhabited or empty
    depending on whether \( b_0 = p^{\Inv}_*(b_0) \) or not.
  \end{proof}
\end{proposition}
...
\begin{lemma}\label{is-embedding-sigma-isolate}
  For all \( A : \Type \), \( B : A \to \Type \), and \( y \JudgeEq ((a, b), \Blank) : \Isolated{(\sum_{A} B)} \),
  there is an equivalence \( \Op{fiber}_{\SigmaIsolate}(y) \simeq \Op{isIsolated}(a) \times \Op{isIsolated}(b) \).
  In particular, fibers of \( \SigmaIsolate_{A, B} \) are propositions, hence it is an embedding.
  \begin{proof}
    ...
    \[
        ...
        \sum_{(b', \Blank) : \Op{singl}(p_{*}{b})}
          \Op{isIsolated}(a') \times \Op{isIsolated}(b')
    \]
    The claim follows by contracting away these singletons.
  \end{proof}
\end{lemma}

A map is an equivalence if and only if it is both an equivalence and a surjection (i.e.\@ a map whose fibers are merely inhabited),
...
\begin{lemma}\label{is-equiv-sigma-isolate-iff-isolated-pair}
  ...
  \begin{proof}
    ...
    By \autoref{is-embedding-sigma-isolate}, the fiber over an arbitrary \( y \JudgeEq ((a, b), h) : \Isolated{(\sum_A\!B)} \) is equivalent to
    \( \Op{isIsolated}(a) \times \Op{isIsolated}(b) \).
