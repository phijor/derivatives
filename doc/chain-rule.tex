...
First, note that \( \SigmaIsolate \) is always an embedding (cf.~\autoref{is-embedding-sigma-isolate}),
hence we can think of \( \Chain_{F,G} \) as embedding \( \Subst{(\Der{F})}{G} \CTimes \Der{G} \)
as a sub-container inside \( \Der(\Subst{F}{G}) \):
\begin{proposition}\label{is-embedding-chain-rule}
  For all \( F, G : \Cont \), the shape map \( \Chain_{F,G}^{\Sh} \) is an embedding.
  \begin{proof}
...
\begin{todo-block}
  Define the notion of \enquote{embedding of containers}.
\end{todo-block}

Second, this strengthens to an equivalence of containers depending on \( \SigmaIsolate \) alone:
...
In an impredicative setting, where \( \Op{Prop} \simeq \Omega \) for some small \( \Omega : \Type \),
either condition implies the law of the excluded middle:
\( \Op{Prop} \) is a set, so \( \Omega \) must be as well.
If \( \Omega \) were discrete, \( P = \top \) would be decidable for any \( P : \Op{Prop} \).
